...
for r in range(gridsize):
    ...
    for c in range(len(cols)):
        current_universe["{}:{}:0".format(c,r)] = cols[c];
...
def get_neighbours(location_key):
    ...
    for i in range(-1,2):
        for j in range(-1,2):
            for k in range(-1,2):
                neighbours.append("{}:{}:{}".format(str(x+i),str(y+j),str(z+k)));
    neighbours.remove("{}:{}:{}".format(x,y,z));
    return neighbours;
...
def apply_cycle_rules(current_universe):
    current_layout = copy.deepcopy(current_universe);
    next_layout = {};
...
for i in range(1,7):    
    # print('ABOUT TO DO ROUND ' + str(i) + ' number of active = ' + str(get_total_active(current_universe)));
    ...
    current_universe = copy.deepcopy(next_universe) ;
